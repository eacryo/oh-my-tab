...
At runtime, the updater loads Sparkle only from `Contents/Frameworks/Sparkle.framework`. The framework is committed at `vendor/Sparkle.framework`; packaging can use `SPARKLE_FRAMEWORK_PATH` to select another source, which `bundle.sh` and `dev-restart.sh` copy into the app bundle. The running app does not read that environment variable. Production packages use `https://download.oh-my-tab.app/appcast.xml` as `SUFeedURL`, while development restart and release scripts use `https://download.oh-my-tab.app/dev_release/appcast.xml`. `SPARKLE_FEED_URL` can override the feed URL at packaging time.
