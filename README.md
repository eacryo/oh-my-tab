...
- <img height="14" src="docs/icons/image.svg"> **Window thumbnails**: Alt-Tab-style cards — caption row (mini app icon + window title + close) above a 16:10 live preview captured via a private WindowServer API; cached frames render instantly and refresh in the background; equal-height, aspect-width cards use balanced centered rows when they fit, while overflowing sets fill leading MRU rows first and scroll continuously. Window count alone chooses the 1.0–1.5× size, so wrapping never resizes cards; spatial arrow-key navigation and Shift+Tab reversal remain stable while scrolling. Requires **Screen Recording** permission — without it the switcher falls back to icon-only cards.
