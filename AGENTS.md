...
- **Move-used-to-top toggle (load-bearing, `clipboard.move_used_to_top`, default ON)**: pasting (select + Enter) writes the entry back to the pasteboard, which the poll re-captures as a re-copy and **moves to the front** (dedup-to-front; Maccy behaves the same). With the toggle OFF, pasting must not reorder the history — the three `write_pasteboard_*` functions stamp a private marker type (`org.oh-my-tab.paste`, like Maccy's `org.p0deje.Maccy`), and `poll_clipboard` skips any change that carries it (`should_skip_paste_writeback(toggle, has_marker)`, checked after the sensitive-marker probe; a real copy clears the pasteboard and thus the marker). Known limitation: an app that rewrites the pasteboard after pasting clears the marker and can still trigger the dedup-move.
