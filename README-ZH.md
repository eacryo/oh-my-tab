...
- **剪贴板历史**(可选,默认关闭):**Option+V** 呼出;记录文本与图片,全局去重(再次复制自动提到最前)、置顶常用条目、单条删除(Backspace 图标/按键)、清除全部(保留置顶)。图片按**原始格式**粘贴(JPG 粘回 JPG、GIF 动图粘回动图,绝不重编码成 PNG)——原始字节存磁盘缓存,大图不占内存;**文件复制只存引用**(从不读取文件内容,粘贴时恢复文件语义,Finder 原样复制文件,同 Windows Win+V / Maccy);浮窗跟随光标显示。
