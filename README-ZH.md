...
- **按内容网格吸附窗口的 App 最大化后会留一条细缝**：这类 App 把窗口尺寸取整到完整的内容单元，无法精确铺满可视区，因此某一边会留下约一个单元高（终端就是一行文本）的空缝，典型例子是终端。这是 App 自己的最大化结果——点绿钮、Option+点绿钮、双击标题栏都一样；请求略大的尺寸或使用系统窗口平铺也都不会改变它。不按网格吸附的 App（访达、浏览器、编辑器等大多数）会精确铺满。
