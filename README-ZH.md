...
> **一条只记一种内容(已知取舍)**——每条历史只记录一种内容:要么**文本**,要么**图片**(图片数据 / 图片文件)。当一次复制**同时带文本和图片**时(例如从网页复制图片,剪贴板上通常还带有文本),只记录**文本**,图片不会被记录。这是有意的 v1 取舍——不同于 Windows Win+V 把剪贴板上的所有格式保存在同一条里、粘贴时由目标应用自行选择支持的格式。
